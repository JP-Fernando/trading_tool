...
struct TickEvent {
    ...
    Volume bid_volume;
    Volume ask_volume;
    Price last;  // Último precio negociado
    Volume last_volume;
...
struct FillEvent {
    ...
    Price fill_price;
    double commission;  // Comisiones del exchange
    double slippage;    // Slippage aplicado
    std::string exchange;
...
struct PnLUpdateEvent {
    ...
    double win_rate() const noexcept {
        return total_trades > 0 ? 
            static_cast<double>(winning_trades) / static_cast<double>(total_trades) : 0.0;
    }
};
...
struct EventComparator {
    bool operator()(const Event& a, const Event& b) const {
        // Remember: std::priority_queue is max-heap
        // and we want min-heap (we don't want to read
        // the future before we read the past)
        // Therefore use '>' to compare
        return get_timestamp(a) > get_timestamp(b); 
    }
};

// Efficient timestamp maker
